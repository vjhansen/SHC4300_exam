...
The power dissipation of the single processor system is `P=C*V^2*f`, with `C` a constant. The parallel processor system can scale the voltage and frequency of each processor to 40% of the original value, now the parallel system has the same performance as the original, single-ARM sequential system. *(See task 1.)*
